...
// ── 7. REDRAW THE ENTIRE CANVAS FROM SCRATCH ──────────────────────
//
//   Called after:
...
// ── 8. STROKE LIFECYCLE (start → continue → end) ─────────────────
...
window.addEventListener('load', () => {
  ...
  // Rotate Prompt logic (Mobile Portrait Only)
  const promptEl = document.getElementById('rotate-prompt');
  if (isPortraitMode && !sessionStorage.getItem('dt_rotate_ok')) {
    promptEl.style.display = 'flex';
  }
  document.getElementById('rotate-ok-btn').addEventListener('click', () => {
    promptEl.style.display = 'none';
    sessionStorage.setItem('dt_rotate_ok', 'true');
  });
});
